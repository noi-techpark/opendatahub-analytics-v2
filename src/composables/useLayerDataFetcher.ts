...
import { loadAlarmConfig, getDefaultAlarmConfigUrl } from '../utils/alarm-config-loader'
...
// removed URL update helper; centralized in useQueryInit
...
export function useLayerDataFetcher() {
  ...
  const filterProvinceBZEvent = (event: EventPoint, date: Date) => {
    ...
    switch (category) {
      ...
      default:
        if (category.includes('Situazione attuale')) return startDayTs !== todayTs
        return false
    }
  }
  ...
  const fetchStationData = async (
      ...
      try {
        ...
        const currentMarkers = opts.filter?.filterString
          ? [...opts.currentMarkers.filter((item) => item.stype !== opts.filter!.stype)]
          : [...opts.currentMarkers]
        ...
        if (flatData && flatData.length > 0) {
          ...
          for (const d of flatData) {
            ...
            const origin = isProvinceEvents ? 'PROVINCE_BZ' : typedDataPoint.sorigin
            ...
            const scode = isProvinceEvents ? typedEventPoint.evuuid : typedDataPoint.scode

            let lastDiffHours: number | undefined = undefined
            let lastMeasurement: (DataPoint & { mperiod: number; dataType: string }) | undefined =
              undefined

            if (!isProvinceEvents && fetchedEvents[fetchedEventsKey] && fetchedEvents[fetchedEventsKey][stype] && fetchedEvents[fetchedEventsKey][stype].stations && fetchedEvents[fetchedEventsKey][stype].stations[scode]) {
              const station = fetchedEvents[fetchedEventsKey][stype].stations[scode]
              for (const dataType in station.sdatatypes) {
                const measurements = station.sdatatypes[dataType].tmeasurements
                if (measurements && measurements.length > 0) {
                  ...
                  if (measurement.mvalidtime) {
                    ...
                    if (lastDiffHours === undefined || diffHours < lastDiffHours) {
                      lastDiffHours = diffHours
                      ;(lastMeasurement as any) = { ...measurement, dataType }
                    }
                  }
                }
              }
            ...
            newPoints.push({
              scode,
              sname: isProvinceEvents ? typedEventPoint.evname : typedDataPoint.sname,
              ...
              coordinates: isProvinceEvents
                ...
                : [typedDataPoint.scoordinate?.x || 0, typedDataPoint.scoordinate?.y || 0],
              infoColor: isProvinceEvents
                ...
                : (lastMeasurement && opts.computeInfoColor)
                  ...
                  : undefined,
              eventData: isProvinceEvents ? (typedEventPoint as any) : undefined,
            })
          }
          ...
          showNotification({
            ...
            message: opts.t('views.map.noDataFor', {
              name: (Array.isArray(layer.stationType) ? layer.stationType : [layer.stationType]).join(', '),
            }),
          })
        }
      ...
      for (const layer of layers) {
        ...
        if (hasFilters) {
          for (const stype of filteredStationTypes) {
            const filterStrings = Object.entries(ctx.selectedFilterOrigins.sorigin)
            ...
            if (stypeFilteredMarkers) newMarkers.push(...stypeFilteredMarkers)
          }

          const unfilteredTypes = stationTypes.filter((s) => !filteredStationTypes.includes(s))
          if (unfilteredTypes.length > 0) {
            const unfilteredLayer = { ...layer, stationType: unfilteredTypes }
  ...
  const getMeasurementsForSelectedLayers = async (
    ...
    const stationInfo: Record<string, { sname: string; coords: [number, number]; stype: string }> = {}
    const measurementsOut: StationMeasurement[] = []

    for (const layer of layers) {
      const stationTypes = Array.isArray(layer.stationType) ? layer.stationType : [layer.stationType]
      let hasFilters = false
      ...
      const fetchFlat = async (stypeList: string[], filterString?: string) => {
        const activeQuery = 'sactive.eq.true'
        let query = 'select=scoordinate%2Cscode%2Cstype%2Csname%2Csorigin&where='
  ...
  const refetchForDataTypeWithFilters = async (
    ...
    const layer = ctx.selectedLayers.find((l) =>
      (Array.isArray(l.stationType) ? l.stationType : [l.stationType]).includes(newVal.stype),
    )
    ...
    return await fetchStationData(layer, {
      currentMarkers: ctx.currentMarkers.filter((m) => m.stype !== newVal.stype),
  ...
  const setLayersToMap = async (
    ...
    if (added) {
      const stationTypes = Array.isArray(added.stationType) ? added.stationType : [added.stationType]
      let hasAppliedFilter = false
      for (const stype of stationTypes) {
        if (ctx.selectedFilterOrigins.sorigin[stype]?.length > 0) {
          const filterString = buildOriginFilterString(ctx.selectedFilterOrigins.sorigin[stype])
          const filtered = await fetchStationData(added, {
            currentMarkers: newMarkersOut.length ? newMarkersOut : ctx.markers,
            filter: { stype, filterString },
            uniqueOrigins: ctx.uniqueOrigins,
            t: ctx.t,
            computeInfoColor: ctx.computeInfoColor,
    ...
    diff.forEach((d) => {
      ...
      if (isProvinceBZ) {
        const keysToRemove = Object.keys(ctx.uniqueOrigins).filter((k) => k.startsWith(d))

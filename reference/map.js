                ...
                for (
                  ...
                  for (
                    ...
                    if (
                      ...
                      (mainData[1] == null ||
                        value_datatype_messurment.mperiod == mainData[1])
                    ) {
                      ...
                      if (
                        ...
                        let value_parts =
                          value_datatype_messurment.mvalue.split(
                            layer_info.imageMapping[is_i].valueSeparator
                          )
                        for (let vp_i = 0; vp_i < value_parts.length; vp_i++) {
                          ...
                          if (imgData.length > 0) {
                            ...
                            mvaluePartImg.src =
                              'data:image/*;base64,' +
                              imgData[0][
                                layer_info.imageMapping[is_i].metaDataImgData
                              ]
                            mvaluePartImg.alt =
                  ...
                  let state = {
                    ...
                    scale: {
                      from: new Date(
                        ...
                        .toISOString()
                        .split('T')[0],
                      to: new Date(value_datatype_messurment.mvalidtime)
                        .toISOString()
                        .split('T')[0],
                    },
                    graphs: [
    ...
    return new Promise(async function (ok, fail) {
      ...
      try {
        ...
        if (
          ...
          raw_marker_overlapping_selected_marker_svg = await fetchSvg_promise(
        ...
        let layer = new ol.layer.Vector({
          ...
          style: function (list) {
            ...
            if (features.length == 1) {
              let isSelected =
                selectedFeature != null && selectedFeature === features[0]
          ...
          for (let i = 1; i < layer_info.icons.length; i++) {
            ...
            if (!datatype_period_duplicates[key]) {
              ...
              query_where_datatypes +=
                (query_where_datatypes === '' ? 'or(' : ',') + query_datatype
            }
          }
          query_where_datatypes += ')'
        ...
        for (var i = 0; i < json_stations_flat.data.length; i++) {
          ...
          var thing = new ol.geom.Point(
            ol.proj.transform([lon, lat], layer_info.projection, 'EPSG:3857')
          )
          ...
          if (json_stations_status[json_stations_flat.data[i].scode]) {
            for (var ic = 1; ic < layer_info.icons.length; ic++) {
              ...
              try {
                ...
                let json_value =
                  json_stations_status[json_stations_flat.data[i].scode]
                    .sdatatypes[cond[1]]
                if (json_value)
        ...
        if (overlapping_star_features.length > 0) {
          for (var i = 0; i < overlapping_star_features.length; i++) {
            let coordinates =
              overlapping_star_features[i][0].getGeometry().flatCoordinates
      ...
      try {
        ...
        for (var i = 0; i < json_stations_flat.data.length; i++) {
          ...
          for (let ci = 0; ci < coordinates.length; ci++) {
            points.push(
              ol.proj.fromLonLat([coordinates[ci][0], coordinates[ci][1]])
            )
          }
